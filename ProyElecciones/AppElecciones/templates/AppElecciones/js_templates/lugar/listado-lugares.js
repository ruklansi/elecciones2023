...
$(function () {
    ...
    var tabla_lugar = $('#tabla').DataTable({
        responsive: true,
        autoWidth: true,
        destroy: true,
        deferRender: true,
        ...
        columns: [
            ...
            { data: 'tipo_lugar__tipo', orderable: false },
            { data: 'autoridad', orderable: false },
            ...
            { data: 'obs', orderable: false },
            {
                ...
                render: function (data, type, row) {
                    let buttons = '';
                    let url1='{% url "actualizar-lugar-interes" 99%}'.replace('99',row.id)
                    let url2='{% url "eliminar-lugar-interes" 99%}'.replace('99',row.id)
                    let url3='{% url "detalles-lugar-interes" 99%}'.replace('99',row.id)
                    if (row.editar){
                    ...
                    buttons += '<a title="Detalle del lugar de interés" href="'+ url3+'" class=""><i class="fas fa-address-card" style="font-size:15px;color:goldenrod"></i></a>';
                    return buttons;
                }
            }
        ],
        language: {
            decimal: "",
            emptyTable: "Sin registros de lugares de interés",
            ...
            search: "Buscar:",
            zeroRecords: "Sin registros de lugar",
            paginate: {
                first: "Primero",
                last: "Ultimo",
                next: "Siguiente",
    ...
    $('select[id="filtro-distrito-lugar"]').on('change', function () {
            comprobarSesion();
