...
from django.db.models import Q, Value, OuterRef, Subquery, BooleanField
from django.db.models.functions import Coalesce

from guardian.shortcuts import get_objects_for_user


def listarParaDatatables(modelo=None, ORDENAR_COLUMNAS=None, buscar=None, agregados=None, columnas=None,
                         otros_filtros=None, con_permisos=False, kwargs=None):
    ...
    if con_permisos:
        queryset = get_objects_for_user(usuario, 'view_' + modelo.__name__.lower(), modelo, accept_global_perms=False).all()

        sub_editar = get_objects_for_user(usuario, 'change_' + modelo.__name__.lower(), modelo,
                                          accept_global_perms=False).filter(id=OuterRef('pk')).annotate(
            edit=Value(True)).values('edit')

        sub_eliminar = get_objects_for_user(usuario, 'delete_' + modelo.__name__.lower(), modelo,
                                            accept_global_perms=False).filter(id=OuterRef('pk')).annotate(
    ...
    if search_value:
        filtro = {}
        palabras=search_value.split(' ')
        for y in buscar:
            for p in palabras:
                filtro[y + '__icontains'] =p
                # print(p)
