...
from AppElecciones.forms import FormVehiculosPropiosSeccion
from AppElecciones.listadoParaDatatebles import listarParaDatatables
from AppElecciones.models import Persona, VehiculosPropiosSeccion, VehiculosPropios, Seccion


class ListarVehiculosPropioEnSeccionesAjax(View):
    def dispatch(self, request, *args, **kwargs):
        return super().dispatch(request, *args, **kwargs)

    def post(self, request, *args, **kwargs):
        id_seccion = request.POST.get('id_seccion', False)
        ...
        lista_veh_propios_seccion = listarParaDatatables(VehiculosPropiosSeccion, ORDENAR_COLUMNAS, buscar, agregados,
                                                         columnas,
                                                         otros_filtros,
                                                         con_permisos, request.POST)
        resultado = dict()
        resultado['data'] = lista_veh_propios_seccion['items']
        resultado['draw'] = lista_veh_propios_seccion['draw']
        resultado['recordsTotal'] = lista_veh_propios_seccion['total']
        resultado['recordsFiltered'] = lista_veh_propios_seccion['count']
        # print(resultado)
        return JsonResponse(resultado, safe=False)


class CrearVehiculoPropioEnSeccion(View):
    def dispatch(self, request, *args, **kwargs):
        return super().dispatch(request, *args, **kwargs)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.method == 'POST':
            ...
            if request.is_ajax():
                form = FormVehiculosPropiosSeccion(request.POST)
                if form.is_valid():
                ...
                data['html_form'] = render_to_string(
                    'AppElecciones/secciones/vehpropio/crear.html', context, request=request)
    ...
    def get(self, request, *args, **kwargs):
        ...
        if request.method == 'GET':
            if request.is_ajax():
                form = FormVehiculosPropiosSeccion()
            context = {'form': form}
            data['html_form'] = render_to_string(
                'AppElecciones/secciones/vehpropio/crear.html', context, request=request)
        return JsonResponse(data)
...
class ActualizarVehiculoPropioEnSeccion(View):
    def dispatch(self, request, *args, **kwargs):
        return super().dispatch(request, *args, **kwargs)
        ...
        if request.method == 'POST':
            if request.is_ajax():
                instancia = get_object_or_404(
                    VehiculosPropiosSeccion, pk=kwargs['pk'])
                ##########################################
                ...
                context = {'form': form}
                data['html_form'] = render_to_string(
                    'AppElecciones/secciones/vehpropio/modificar.html', context, request=request)
        return JsonResponse(data)
    ...
    def get(self, request, *args, **kwargs):
        ...
        if request.method == 'GET':
            if request.is_ajax():
                form = FormVehiculosPropiosSeccion(instance=instancia)
        context = {'form': form}
        data['html_form'] = render_to_string(
            'AppElecciones/secciones/vehpropio/modificar.html', context, request=request)
        return JsonResponse(data)
...
class EliminarVehiculosPropiosEnSeccion(View):
    def dispatch(self, request, *args, **kwargs):
        return super().dispatch(request, *args, **kwargs)
