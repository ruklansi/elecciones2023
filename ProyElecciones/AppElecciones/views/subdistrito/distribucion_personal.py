...
from AppElecciones.forms import FormDistribucionPersonalDistrito, FormDistribucionPersonalSubdistrito
from AppElecciones.listadoParaDatatebles import listarParaDatatables
from AppElecciones.models import Persona, DistribucionPersonalSubdistrito, Subdistrito


class ListarDistribucionSubdistritoAjax(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        org_subdistrito = listarParaDatatables(DistribucionPersonalSubdistrito, ORDENAR_COLUMNAS, buscar, agregados,
                                               columnas,
                                               otros_filtros,
                                               con_permisos, request.POST)
        resultado = dict()
        resultado['data'] = org_subdistrito['items']
        resultado['draw'] = org_subdistrito['draw']
        resultado['recordsTotal'] = org_subdistrito['total']
        resultado['recordsFiltered'] = org_subdistrito['count']
        # print(resultado)
        return JsonResponse(resultado, safe=False)


class CrearDistribucionEnSubdistrito(View):
    def dispatch(self, request, *args, **kwargs):
        return super().dispatch(request, *args, **kwargs)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.method == 'POST':
            ...
            if request.is_ajax():
                ...
                ####################
            form = FormDistribucionPersonalSubdistrito(request.POST)
            if form.is_valid():
                ...
                data['html_form'] = render_to_string(
                    'AppElecciones/subdistritos/distribucionPersonal/crear.html', context, request=request)
    ...
    def get(self, request, *args, **kwargs):
        ...
        if request.method == 'GET':
            if request.is_ajax():
                form = FormDistribucionPersonalSubdistrito()
        context = {'form': form}
        data['html_form'] = render_to_string(
            'AppElecciones/subdistritos/distribucionPersonal/crear.html', context, request=request)
        return JsonResponse(data)
...
class ActualizarDistribucionEnSubdistrito(View):
    def dispatch(self, request, *args, **kwargs):
        return super().dispatch(request, *args, **kwargs)
        ...
        if request.method == 'POST':
            if request.is_ajax():
                instancia = get_object_or_404(
                    DistribucionPersonalSubdistrito, pk=kwargs['pk'])
                ####################################
                ...
                if form.is_valid():
                    ...
                    context = {'form': form}
                    data['html_form'] = render_to_string(
                        'AppElecciones/subdistritos/distribucionPersonal/modificar.html', context, request=request)
...
class EliminarDistribucionEnSubdistrito(View):
    def dispatch(self, request, *args, **kwargs):
        return super().dispatch(request, *args, **kwargs)
