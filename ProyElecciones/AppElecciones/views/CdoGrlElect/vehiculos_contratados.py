from django.db.models import Value, CharField, F, When, Case
from django.db.models.functions import Concat
from model_utils import Choices
from AppElecciones.listadoParaDatatebles import listarParaDatatables
from AppElecciones.models import CdoGrlElect, VehiculosContratadosCdoGrlElect, Persona, VehiculosContratados
from AppElecciones.forms import FormVehiculosContratadosCdoGrlElect
...
class ListarVehiculosContratadosCdoGrlElectAjax(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        buscar = ['vehiculo_contratado__tipo_vehiculo_contratado__tipo_vehiculo_civi', 'zona_trabajo',
                  'cond_veh_cont', 'vehiculo_contratado__patente_matricula']
        ...
        con_permisos = True
        lista_veh_contratados = listarParaDatatables(VehiculosContratadosCdoGrlElect, ORDENAR_COLUMNAS, buscar,
                                                     agregados,
                                                     columnas,
                                                     otros_filtros,
        ...
        resultado = dict()
        resultado['data'] = lista_veh_contratados['items']
        resultado['draw'] = lista_veh_contratados['draw']
        resultado['recordsTotal'] = lista_veh_contratados['total']
        resultado['recordsFiltered'] = lista_veh_contratados['count']
        # print(resultado)
        return JsonResponse(resultado, safe=False)


class CrearVehiculoContratadoEnCdoGrlElect(View):
    def dispatch(self, request, *args, **kwargs):
        return super().dispatch(request, *args, **kwargs)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.method == 'POST':
            id_del_cge = request.POST['id_cge']
            if request.is_ajax():
                form = FormVehiculosContratadosCdoGrlElect(request.POST)
                if form.is_valid():
                    ################################
                    ...
                    if veh_cont:
                        VehiculosContratados.objects.filter(id=veh_cont).update(tiene_destino=True, cantidad_empleos=F(
                            'cantidad_empleos') + 1)
                    #################################
                    instancia_form = form.save(commit=False)
                    CdoGrlElect_ = CdoGrlElect.objects.get(id=id_del_cge)
                    instancia_form.cge = CdoGrlElect_
                    instancia_form.save()
                ...
                data['html_form'] = render_to_string(
                    'AppElecciones/CdoGrlElect/crearVehCont.html', context, request=request)
    ...
    def get(self, request, *args, **kwargs):
        ...
        if request.method == 'GET':
            if request.is_ajax():
                form = FormVehiculosContratadosCdoGrlElect()
                context = {'form': form}
                data['html_form'] = render_to_string(
                    'AppElecciones/CdoGrlElect/crearVehCont.html', context, request=request)
        return JsonResponse(data)
...
class ActualizarVehiculoContratadoEnCdoGrlElect(View):
    def dispatch(self, request, *args, **kwargs):
        return super().dispatch(request, *args, **kwargs)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.method == 'POST':
            if request.is_ajax():
                instancia = get_object_or_404(VehiculosContratadosCdoGrlElect, pk=kwargs['pk'])
                ################################
                ...
                #################################
                form = FormVehiculosContratadosCdoGrlElect(request.POST, instance=instancia)
                if form.is_valid():
                    ...
                    context = {'form': form}
                    data['html_form'] = render_to_string(
                        'AppElecciones/CdoGrlElect/modificarVehCont.html', context, request=request)
    ...
    def get(self, request, *args, **kwargs):
        ...
        instancia = get_object_or_404(
            VehiculosContratadosCdoGrlElect, pk=kwargs['pk'])
        if request.method == 'GET':
            if request.is_ajax():
                form = FormVehiculosContratadosCdoGrlElect(instance=instancia)
                context = {'form': form}
                data['html_form'] = render_to_string(
                    'AppElecciones/CdoGrlElect/modificarVehCont.html', context, request=request)
        return JsonResponse(data)
...
class EliminarVehiculosContratadosEnCdoGrlElect(View):
