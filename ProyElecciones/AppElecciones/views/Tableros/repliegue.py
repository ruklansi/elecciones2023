...
df['inicio_repliegue']=round(df['ini_repl']*100/df['total'],2)
...
def crear_mapa(df, zoom):
    ...
    mapa = px.choropleth_mapbox(
        ...
        geojson=df.geom,
        locations=df.index,
        hover_data={'id': False, "departamen": True, "replegado": True, "total": True, 'repliegue': True},
        color='repliegue',
    ...
    mapa.update_traces(
        hovertemplate='<b>%{customdata[1]} %{customdata[4]}% </b><br>Replegados:%{customdata[2]}<br>Total:%{customdata[3]}'
    )
...
def crear_porcentaje(df, graf_rep):
    if graf_rep=='rep':
        ...
        data=round(df['inicio_repliegue'].mean(skipna = True),2)    
        titulo='INICIARON EL REPLIEGUE'
    fig = go.Figure(
        go.Indicator(
            mode='gauge+number',
            value=data,
...
def crear_indicadores(df_data,accion):
    ...
    if accion=='desplegados':
        ...
        titulo='DESPLEGADOS'   
    if accion=='ini':
        valor=data['ini_repl'].sum()
        titulo='INICIARON REPLIEGUE' 
        color='#e74c3c'
    if accion=='reple':
        valor=data['replegado'].sum()
        titulo='REPLEGADOS'  
        color= '#18c044'       
...
app = DjangoDash(name='Repliegue', external_stylesheets=[dbc.themes.DARKLY])
todos = [{"label": 'TODOS', "value": 'TODOS'}]
app.layout = html.Div(
        [   
            dbc.Row([html.H1('REPLIEGUE DEL PERSONAL DE LOS LOCALES', style={'textAlign': 'center', 'background-color': '#267300'})]),
            ...
            dbc.Row([dbc.Col([dbc.Row([dcc.Loading(id="ls-loading-2", children=[dcc.Graph(id='grafico2', figure=crear_porcentaje(df,'ini'))], type="default")]),
                              
                              dbc.Row([dcc.Loading(id="ls-loading-6", children=[dcc.Graph(id='grafico1', figure=crear_porcentaje(df,'rep'))], type="default")])
                              ], style={"height": "100%", }
                             , md=3),
                     dbc.Col([dbc.Row([dcc.Loading(id="ls-loading-3", children=[dcc.Graph(id='mapa', figure=crear_mapa(df,3))], type="default")])], style={"height": "100%", }, md=6),

                     dbc.Col([dbc.Row([dcc.Loading(id="ls-loading-4", children=[dcc.Graph(id='total',figure=crear_indicadores(df,'total') )], type="default")]),
                              dbc.Row([dcc.Loading(id="ls-loading-5", children=[dcc.Graph(id='ini', figure=crear_indicadores(df,'ini'))], type="default")]),
...
def actualizar_mapa(distrito,intervalo,**kwargs):
    ...
    if distrito and distrito != 'TODOS':
        ...
        datos_mapa['repliegue']=round(datos_mapa['replegado']*100/datos_mapa['total'],2)
        datos_mapa['inicio_repliegue']=round(datos_mapa['ini_repl']*100/datos_mapa['total'],2)
        ...
        datos_mapa = geopandas.GeoDataFrame.from_postgis(sql, conn)
        datos_mapa.to_crs(epsg=4326, inplace=True)
        datos_mapa['repliegue']=round(datos_mapa['replegado']*100/df['total'],2)
        datos_mapa['inicio_repliegue']=round(datos_mapa['ini_repl']*100/datos_mapa['total'],2)       
        esta_urnas=estado_urnas('TODOS') 
    return crear_mapa(datos_mapa,zoom),crear_indicadores(datos_mapa,'total'),crear_indicadores(datos_mapa,'reple'),crear_indicadores(datos_mapa,'ini'),crear_porcentaje(datos_mapa,'rep'),crear_porcentaje(datos_mapa,'ini'),esta_urnas[0],esta_urnas[1],esta_urnas[2]
